#!/usr/bin/env python3
"""
Extract individual frames from the Excalidraw SVG by creating copies with modified viewBox.
This preserves all SVG content but changes the visible area.
"""
...
def extract_frames_direct():
    """Extract individual frames by modifying the viewBox of the original SVG."""
    ...
    for frame_id, frame_info in FRAME_COORDS.items():
        ...
        # Create a copy of the SVG with modified viewBox
        ...
        print(f"Created frame: {output_file.name} (viewBox: {view_x} {view_y} {view_width} {view_height})")
